...
import { teamQueryOptions } from "@/queries/team-query";
...
export function TeamTable() {
  ...
  const { data: teamsPaginated, isError } = useSuspenseQuery(
    teamQueryOptions(searchParams.toString())
  );
            ...
            <TableRow key={team.id}>
              ...
              <TableCell>
                <div className="flex flex-row h-full justify-center">
                  <Link href={`/dashboard/book/edit/${team.id}`}>
                  ...
                  <UserDeleteButton
                    // id={user.id}
                    callback={() => {
                      console.log("delete user", team.id);
                    }}
                  />
                </div>
              </TableCell>
